...
export default function Dashboard({ view, sketchLayer, estados, municipios, biomas }: Props) {
  ...
  useEffect(() => {
    ...
    sync();
    const handle = sketchLayer.graphics.on("change", sync);
    return () => handle.remove();
  }, [sketchLayer]);

  const queryLayer = async (layer: FeatureLayer, geometry: any) => {
  ...
  const runAnalysis = async () => {
    const graphic = sketchLayer.graphics.getItemAt(sketchLayer.graphics.length - 1);
    ...
    try {
      ...
      const areaKm2 = Math.abs(geometryEngine.geodesicArea(geometry, "square-kilometers"));
      ...
      setResult({ areaKm2, estados: ufs, municipios: muns, biomas: bios });
  ...
  return (
    <div className="dashboard">
      ...
      <div className="dashboard-actions">
        <button className="btn" disabled={!hasGeometry || loading} onClick={runAnalysis}>
        ...
        {result && (
          <>
            <div className="card">
              ...
              <ul>
                {result.estados.map((e, i) => <li key={i}>{e.nm_uf}</li>)}
              </ul>
            </div>
            ...
            <div className="card">
              ...
              <ul>
                {result.municipios.map((m, i) => <li key={i}>{m.nm_mun}</li>)}
              </ul>
            </div>
            ...
            <div className="card">
              ...
              <ul>
                {result.biomas.map((b, i) => <li key={i}>{b.NOME}</li>)}
              </ul>
            </div>
          </>
        )}
